...
export function getServerConfig(env: NodeJS.ProcessEnv = process.env): ServerConfig {
  ...
  if (sessionEncryptionKey.equals(tokenEncryptionKey)) {
    throw new Error('SESSION_ENCRYPTION_KEY 與 TOKEN_ENCRYPTION_KEY 必須使用不同金鑰。')
  }
  ...
  const firestoreCredentials = parseFirestoreCredentials(
    required(env, 'FIRESTORE_SERVICE_ACCOUNT_JSON'),
    firestoreProjectId,
  )
  const legacyMigrationSecret = validateSecret(
    requiredSecret(env, 'LEGACY_MIGRATION_SECRET'),
    'LEGACY_MIGRATION_SECRET',
  )
  const cronSecret = validateSecret(requiredSecret(env, 'CRON_SECRET'), 'CRON_SECRET')
  if (legacyMigrationSecret === cronSecret) {
    throw new Error('LEGACY_MIGRATION_SECRET 與 CRON_SECRET 不得使用相同值。')
  }
...
function requiredSecret(env: NodeJS.ProcessEnv, name: string): string {
  ...
  if (!value?.trim()) throw new Error(`缺少伺服器端環境變數：${name}`)
  return value
}
...
function parseFirestoreCredentials(jsonString: string, expectedProjectId: string): FirestoreCredentials {
  try {
    ...
    const privateKey = typeof parsed.private_key === 'string' && parsed.private_key.trim() ? parsed.private_key : ''
...
function validateSecret(secret: string, name: string): string {
  if (!/^[A-Za-z0-9_-]{32,}$/.test(secret)) {
    throw new Error(`${name} 必須是至少 32 個字元的 base64url ASCII 隨機值。`)
  }
  return secret
}
