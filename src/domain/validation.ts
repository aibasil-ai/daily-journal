import type { EntryInput, JournalLink } from './journal'
...
export function validateEntryInput(
  ...
  if (!activeCategoryIds.has(input.categoryId)) {
    issues.push({ field: 'categoryId', message: zhTW.validation.category })
  }
  for (const link of input.links) {
    if (!isCompleteValidLink(link)) {
...
function isCompleteValidLink(link: JournalLink): boolean {
  return Boolean(link.label.trim()) && isHttpUrl(link.url.trim())
}
